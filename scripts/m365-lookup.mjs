...
if (args[0] === '-f' || args[0] === '--find') {
  ...
  if (!hits.length) { console.error(`Nothing matches "${needle}".`); process.exit(1); }
  for (const h of hits) console.log(`m365 ${h.name}${h.why ? `   (${h.why})` : ''}`);
  process.exit(0);
}
...
if (wantedSection) {
  ...
  if (text === '') { console.error(`m365 ${canonicalName} documents no ${wantedSection}.`); process.exit(1); }
  console.log(text);
  process.exit(0);
}
